...
#include <conio.h>
#include <map>
#include "json.hpp" // Assurez-vous d'avoir la bibliothèque nlohmann/json incluse
	...
	if (uMsg == WM_SOCKET)
	{
		...
		else
		{
			...
			case FD_WRITE:
				...
				// Check if player is equal to 1
				if (SocketInfo->BytesRECV == 0 && SocketInfo->Buffer[0] == '1') {
					// Player 1 has played at casePos, send message to clients
					json sendData;
					sendData["player"] = g_player;
					sendData["casePos"] = g_casePos; // Assuming casePos is defined somewhere in your code
					std::string message = sendData.dump(); // Convert JSON to string
					// Prepare the send buffer
					strcpy(SocketInfo->Buffer, message.c_str());
					SocketInfo->DataBuf.buf = SocketInfo->Buffer;
